...
var targetUnions = points.size - 1
var lastUnionPairX = 0L to 0L
for(i in 0 until pointPairs.size) {
    val curPair = pointPairs[i]
    if(dsu.union(curPair.a, curPair.b)) {
        lastUnionPairX = points[curPair.a].x to points[curPair.b].x
        targetUnions--

        if(targetUnions <= 0) break;
    }
}

println(lastUnionPairX.first * lastUnionPairX.second)
